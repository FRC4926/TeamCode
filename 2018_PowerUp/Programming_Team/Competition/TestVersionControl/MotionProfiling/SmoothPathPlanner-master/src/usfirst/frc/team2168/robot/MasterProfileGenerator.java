 ...
 *
 */
public class MasterProfileGenerator
{
	 ...
	 * @param path
	 */
	public MasterProfileGenerator(double[][] path)
	{
...
public static void getCSV(double[][] path, String fileName, double timeStep) throws Exception //saves the files to T: drive in the project folder
	{		
		...
		for(double[] Row : path)
		{
			...
			for(double ColValue : Row)
			{
				...
				//System.out.print("i am now here");
				if (i == 1)
						{
							writer.write(Double.toString(ColValue)+"," );
							writer.write(CurrPos +",");
							CurrPos = CurrPos+(timeStep*ColValue);
						}
			...
			//System.out.print("\n");	
			writer.write("20,"); //need to make it a parameter but insert as int and convert to string
			writer.write("\n");
		...
		catch(IOException ex)
		{
			...
			System.out.print("i shouldnt be here");
		}
	...
	public static void main(String[] args)
	{
		...
		final MasterProfileGenerator path = new MasterProfileGenerator(waypoints);
		...
		System.out.println(path.smoothRightVelocity);

		//example on printing useful path information
		//System.out.println(path.numFinalPoints);
		//System.out.println(path.pathAlpha);

		//final double[][] rightFinalPath = rightPath;
		//double[][] rightVels = path.smoothRightVelocity;
		//path.getCSV(rightVels, "RightVelPoints");
		try {
			getCSV(path.smoothRightVelocity, "Path_10_Right_v9.csv", timeStep);
			getCSV(path.smoothLeftVelocity, "Path_10_Left_v9.csv", timeStep);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println(path.rightPath);
	...
	/*public void poofExample()
	{
		...
		final MasterProfileGenerator path = new MasterProfileGenerator(CheesyPath);
		path.calculate(totalTime, timeStep, robotTrackWidth);
